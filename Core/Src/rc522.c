...
uchar MFRC522_ReadRegister(uchar addr) {
    uchar value;
    HAL_GPIO_WritePin(MFRC522_CS_PORT, MFRC522_CS_PIN, GPIO_PIN_RESET); // Select the MFRC522

    uchar command = ((addr << 1) & 0x7E) | 0x80; // Read command
    HAL_SPI_Transmit(HSPI_INSTANCE, &command, 1, HAL_MAX_DELAY);
    HAL_SPI_Receive(HSPI_INSTANCE, &value, 1, HAL_MAX_DELAY);

    HAL_GPIO_WritePin(MFRC522_CS_PORT, MFRC522_CS_PIN, GPIO_PIN_SET); // Deselect the MFRC522
    return value;
}
